...
#define MY_CUSTOM_COMMAND 1 // for the ioctl function 
...
//WRITE copies data from the user space to the kernel space buffer (ece_buffer): returns number of bytes written
// Parameters:  *fp = pointer to file structure (like a file descriptor), *buf = pointer to user space buffer w data to be written,
// count = number of bytes to be written, loff_t *op = pointer to the file offset, indicating where the write operation should start
...
static long ece_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case MY_CUSTOM_COMMAND:
            ...
            printk(KERN_INFO " ECE 4310: Custom ioctl command received\n");
...

{
    ...
    int ret = 0;
    major = register_chrdev(0, "test_mod01", &ece_fops); // passign &ece_fops associates file operations structure with the character device
    ece_offset_w = 0;
    ece_offset_r = 0;
    ece_size = 0;
