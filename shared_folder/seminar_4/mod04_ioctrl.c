...
static char ece_buffer[ECE_BUF_SIZE]; // total capacity of buffer
...
static ssize_t ece_write(struct file *fp, const char *buf, size_t count, loff_t *op) 
{
    ...
    ret = copy_from_user(&ece_buffer[ece_offset_w], buf, count); // number of bytes not successfully copied
    if (ret !=0)
    {
...
static ssize_t ece_read(struct file *fp, // pointer to file structure (file descriptor)
    char *buf,          /* to fill with data for user: pointer to user space buffer! */
    ...
    loff_t *offset)     /* take the data from offset */
{
    ...
    ret = copy_to_user(buf, &ece_buffer[ece_offset_r], count); // ret = number of bytes not successfully copied
    if (ret != 0)
    {
...
{
    switch (cmd) {
        case RESET_READ_OFFSET:
            ece_offset_r = 0;
            printk(KERN_INFO " ECE 4310: Read Offset has been reset. \n");
            break;
        case RESET_WRITE_OFFSET:
            ece_offset_w = 0;
            printk(KERN_INFO " ECE 4310: Write Offset has been reset. \n");
            break;    
        default:
            return -1;  
    }
...

{
    ...
    int ret = 0;
    major = register_chrdev(0, "mod04_ioctrl", &ece_fops); // assign &ece_fops associates file operations structure with the character device
    ece_offset_w = 0;
    ece_offset_r = 0;
    ece_size = 0;
...
void ece_end(void) //unregisters device and releases all resources we were using
{
    if (isReg) //if (major >=0)
    {
        unregister_chrdev(major, "mod04_ioctrl");
    }
